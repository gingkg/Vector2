...
template <typename T> void traverse2(Vector<Vector<T> > const& V)//遍历输出二维数组
{
...
template <typename T>
Vector<Vector<T> > & Vector<T>::operator=(Vector<Vector<T> > const& V)//重载赋值操作符
{
	for(int i=0;i<V.size();i++){
	if(_elem) delete [] _elem;//释放原有内容
	copyFrom(V[i]._elem,0,V[i].size());//整体复制
	return *this;//返回当前对象的引用，以便链式赋值
	}
}
...
//cout<<(V1+V2)[3];
    ...
    V1.sort(0,V1.size());//归并排序
    V1.search(20,0,V1.size());//A,B,C查找
...
//实现在维数组中插入行
	cout<<"正在插入行...."<<endl<<V3.insert(2,Vector<int>(6,3,6))<<endl<<"插入成功"<<endl;
	traverse2(V3);
//实现在二维数组中插入列
    cout<<"正在插入列...."<<endl;
	for(int i=0;i<V3.size();i++)
...
//实现二维数组的逐行查找
	for(int i=0;i<V3.size();i++)
	  {
	    cout<<"第"<<i+1<<"行的查找结果是："<<V3[i].find(6)<<endl;
	  }
//实现二维向量整体的加减乘除
	traverse2(V3);
	operation(V3,1,2);//+2
